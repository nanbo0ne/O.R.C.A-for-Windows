...
> **3.0.13** keeps readable chat history when conversation context is condensed, improves context-usage reporting, and adds a live work overview. Guidance is clearer, with improvements to image and file handling, pasting, and permission dialogs on smaller windows. The overview shows only activity the app actually observes.
>
> [Release notes](docs/releases/desktop-v3.0.13.md) · [Build checklist](docs/build/desktop-v3.0.13.md) · [Release verification pending](docs/audits/desktop-v3.0.13-verification.md)
...
| --- | --- | --- |
| Windows x64 | [Installer](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.13/O.R.C.A-for-Windows-windows-amd64-installer.exe) · [Portable ZIP](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.13/O.R.C.A-for-Windows-windows-amd64.zip) | Cloud features; managed local AI and Computer Use temporarily disabled |
| macOS 12+ Universal | [DMG](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.13/O.R.C.A-macos-universal.dmg) | Intel and Apple Silicon; managed local AI and Computer Use temporarily disabled |
| Linux x64 | [DEB](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.13/O.R.C.A-linux-amd64.deb) | Debian / Ubuntu; requires a matching WebKitGTK runtime; managed local AI and Computer Use temporarily disabled |

- [3.0.13 release page, checksums, and release notes](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/tag/desktop-v3.0.13)
...
- Provider corrections from 3.0.6/3.0.7 remain: the displayed protocol matches the saved value, backend Agent endpoints are normalized, and protocol/endpoint mismatches give actionable guidance to check the protocol and Base URL without switching credentials to another provider. These are not new protocol fixes in 3.0.13.
...
- **3.0.5 migration compatibility:** upgrades from older versions retain the previously introduced one-time migration of official DeepSeek defaults, model roles, and saved session selections to V4.1 Flash, including old official Pro choices. 3.0.13 does not repeat completed migration, and deliberate Pro selections made afterward remain. Custom providers, proxy endpoints, separate credentials, and non-official model choices stay isolated. Matching model names alone never trigger cross-provider migration; historical messages and stored amounts are not rewritten.
...
- Image bytes are not embedded in session JSONL. Sessions may retain attachment references and display metadata; accepted immutable image snapshots are stored under `.orca/attachments` for thumbnails and reopening. Images are current-turn input by default and are still sent to the enabled vision provider. Size, count, format, and model capability limit availability.
...
3.0.13 keeps Computer Use disabled on **Windows, macOS, and Linux**. It registers no computer-control tools, captures no screens, and performs no native mouse, keyboard, or window-control actions. ORCA Agent, automation, bots, and subtasks cannot start this feature.
...
- Tool-summary merging from 3.0.6/3.0.7 continues to retain actual stage text and order, without generic replacements or lost intermediate replies. 3.0.13 does not reimplement that logic or change the Classic layout and control arrangement.
...
| Local AI is missing | Managed runtime and downloads remain temporarily disabled in 3.0.13; old configuration cannot enable them. Files remain and custom external services still work |
...
See the [build checklist](docs/build/desktop-v3.0.13.md) for build instructions. Release acceptance remains pending; see the [verification record](docs/audits/desktop-v3.0.13-verification.md) after the release gates are completed. See [desktop/README.md](desktop/README.md) for desktop details and the [artifact runtime boundary](docs/ARTIFACT_RUNTIME.md) for office artifacts.
