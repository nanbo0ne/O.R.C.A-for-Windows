...
> **3.0.12** fixes erroneous end-of-turn failures caused by recovered tool actions or rejected checklist updates, and improves stream completion, cancellation, and slow-consumer handling. Unfinished tasks and real failures remain visible. It retains the compaction, Composer, and session-loading fixes from 3.0.11, the standard Windows installer, and Modern / Classic layouts.
>
> [Release notes](docs/releases/desktop-v3.0.12.md) · [Build checklist](docs/build/desktop-v3.0.12.md) · [Release verification](docs/audits/desktop-v3.0.12-verification.md)
...
| --- | --- | --- |
| Windows x64 | [Installer](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.12/O.R.C.A-for-Windows-windows-amd64-installer.exe) · [Portable ZIP](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.12/O.R.C.A-for-Windows-windows-amd64.zip) | Cloud features; managed local AI and Computer Use temporarily disabled |
| macOS 12+ Universal | [DMG](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.12/O.R.C.A-macos-universal.dmg) | Intel and Apple Silicon; managed local AI and Computer Use temporarily disabled |
| Linux x64 | [DEB](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.12/O.R.C.A-linux-amd64.deb) | Debian / Ubuntu; requires a matching WebKitGTK runtime; managed local AI and Computer Use temporarily disabled |

- [3.0.12 release page, checksums, and release notes](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/tag/desktop-v3.0.12)
...
- Provider corrections from 3.0.6/3.0.7 remain: the displayed protocol matches the saved value, backend Agent endpoints are normalized, and protocol/endpoint mismatches give actionable guidance to check the protocol and Base URL without switching credentials to another provider. These are not new protocol fixes in 3.0.12.
...
- **3.0.5 migration compatibility:** upgrades from older versions retain the previously introduced one-time migration of official DeepSeek defaults, model roles, and saved session selections to V4.1 Flash, including old official Pro choices. 3.0.12 does not repeat completed migration, and deliberate Pro selections made afterward remain. Custom providers, proxy endpoints, separate credentials, and non-official model choices stay isolated. Matching model names alone never trigger cross-provider migration; historical messages and stored amounts are not rewritten.
...
3.0.12 keeps Computer Use disabled on **Windows, macOS, and Linux**. It registers no computer-control tools, captures no screens, and performs no native mouse, keyboard, or window-control actions. ORCA Agent, automation, bots, and subtasks cannot start this feature.
...
- Tool-summary merging from 3.0.6/3.0.7 continues to retain actual stage text and order, without generic replacements or lost intermediate replies. 3.0.12 does not reimplement that logic or change the Classic layout and control arrangement.
...
| Local AI is missing | Managed runtime and downloads remain temporarily disabled in 3.0.12; old configuration cannot enable them. Files remain and custom external services still work |
...
See the [build checklist](docs/build/desktop-v3.0.12.md) for build instructions and the [detailed verification record](docs/audits/desktop-v3.0.12-verification.md) for coverage, release checks, and native-window interaction testing deferred at the user's request. See [desktop/README.md](desktop/README.md) for desktop details and the [artifact runtime boundary](docs/ARTIFACT_RUNTIME.md) for office artifacts.
