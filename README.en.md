...
> **3.0.9** restores the standard Windows installer wizard, improves process shutdown and upgrade checks, and preserves drafts and sessions. Application features and Modern / Classic layouts are unchanged.
>
> [Release notes](docs/releases/desktop-v3.0.9.md) · [Build checklist](docs/build/desktop-v3.0.9.md) · [Release verification](docs/audits/desktop-v3.0.9-verification.md)

## Downloads

Download 3.0.9 packages, signatures, and SHA-256 checksums from the Release links below. New builds omit old-brand duplicate assets while retaining installer upgrade compatibility.

| Platform | Package | Notes |
| --- | --- | --- |
| Windows x64 | [Installer](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.9/O.R.C.A-for-Windows-windows-amd64-installer.exe) · [Portable ZIP](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.9/O.R.C.A-for-Windows-windows-amd64.zip) | Cloud features; managed local AI and Computer Use temporarily disabled |
| macOS 12+ Universal | [DMG](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.9/O.R.C.A-macos-universal.dmg) | Intel and Apple Silicon; managed local AI and Computer Use temporarily disabled |
| Linux x64 | [DEB](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.9/O.R.C.A-linux-amd64.deb) | Debian / Ubuntu; requires a matching WebKitGTK runtime; managed local AI and Computer Use temporarily disabled |

- [3.0.9 release page, checksums, and release notes](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/tag/desktop-v3.0.9)
...
- Provider corrections from 3.0.6/3.0.7 remain: the displayed protocol matches the saved value, backend Agent endpoints are normalized, and protocol/endpoint mismatches give actionable guidance to check the protocol and Base URL without switching credentials to another provider. These are not new protocol fixes in 3.0.9.
...
- **3.0.5 migration compatibility:** upgrades from older versions retain the previously introduced one-time migration of official DeepSeek defaults, model roles, and saved session selections to V4.1 Flash, including old official Pro choices. 3.0.9 does not repeat completed migration, and deliberate Pro selections made afterward remain. Custom providers, proxy endpoints, separate credentials, and non-official model choices stay isolated. Matching model names alone never trigger cross-provider migration; historical messages and stored amounts are not rewritten.
...
3.0.9 keeps Computer Use disabled on **Windows, macOS, and Linux**. It registers no computer-control tools, captures no screens, and performs no native mouse, keyboard, or window-control actions. ORCA Agent, automation, bots, and subtasks cannot start this feature.
...
- Tool-summary merging from 3.0.6/3.0.7 continues to retain actual stage text and order, without generic replacements or lost intermediate replies. 3.0.9 does not reimplement that logic or change the Classic layout and control arrangement.
...
| Local AI is missing | Managed runtime and downloads remain temporarily disabled in 3.0.9; old configuration cannot enable them. Files remain and custom external services still work |
...
See the [3.0.9 build checklist](docs/build/desktop-v3.0.9.md) and [release verification](docs/audits/desktop-v3.0.9-verification.md) for results and coverage limits. Preview CI [35320328576](https://github.com/nanbo0ne/O.R.C.A-for-Windows/actions/runs/35320328576) passed isolated Windows x64 installation, upgrade from 3.0.8, and default-uninstall checks, preserving synthetic configuration, session, draft, and model data. This does not establish manual acceptance of every platform's installer UI or the complete draft-saving exit flow in a real app window, nor replace formal-build acceptance. The [3.0.8 UI audit](docs/audits/2026-09-18-modern-chat-polish.md) remains historical evidence. See [desktop/README.md](desktop/README.md) for desktop and platform details and the [artifact runtime boundary](docs/ARTIFACT_RUNTIME.md) for office artifacts.
