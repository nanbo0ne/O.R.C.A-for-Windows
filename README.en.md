...
> **3.0.6: navigation, tool summaries, and custom-provider corrections.** This patch focuses on the Modern navigation gutter and compact rail, consecutive tool summaries that retain actual stage text, consistent custom-provider protocol selection, and backend Agent endpoint normalization with actionable configuration errors. Assistant, Coding, ORCA Agent, images and files, artifacts, multiple providers, memory, and automation remain; the official DeepSeek default is still V4.1 Flash. There is no layout redesign and Classic is unchanged. Managed local AI and Computer Use stay disabled on every platform, with code, configuration, and model files retained. See the [bilingual release notes](docs/releases/desktop-v3.0.6.md) and [verification record](docs/audits/desktop-v3.0.6-verification.md).

## Downloads

Download files, signatures and SHA-256 checksums from the 3.0.6 Release below. New builds omit old-brand duplicate assets while retaining installer upgrade compatibility.

| Platform | Package | Notes |
| --- | --- | --- |
| Windows x64 | [Installer](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.6/O.R.C.A-for-Windows-windows-amd64-installer.exe) · [Portable ZIP](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.6/O.R.C.A-for-Windows-windows-amd64.zip) | Cloud features; managed local AI and Computer Use temporarily disabled |
| macOS 12+ Universal | [DMG](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.6/O.R.C.A-macos-universal.dmg) | Intel and Apple Silicon; managed local AI and Computer Use temporarily disabled |
| Linux x64 | [DEB](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/download/desktop-v3.0.6/O.R.C.A-linux-amd64.deb) | Debian / Ubuntu; requires a matching WebKitGTK runtime; managed local AI and Computer Use temporarily disabled |

- [3.0.6 release page, checksums, and release notes](https://github.com/nanbo0ne/O.R.C.A-for-Windows/releases/tag/desktop-v3.0.6)
...
- The 3.0.6 provider correction keeps the displayed protocol and saved value aligned: OpenAI-compatible must not silently save Anthropic because it is first in the alphabetically sorted registry. Backend Agent endpoints are normalized, and protocol/endpoint mismatches give actionable guidance to check the protocol and Base URL without switching credentials to another provider.
...
- **3.0.5 migration compatibility:** upgrades from older versions retain the previously introduced one-time migration of official DeepSeek defaults, model roles, and saved session selections to V4.1 Flash, including old official Pro choices. 3.0.6 does not repeat completed migration, and deliberate Pro selections made afterward remain. Custom providers, proxy endpoints, separate credentials, and non-official model choices stay isolated. Matching model names alone never trigger cross-provider migration; historical messages and stored amounts are not rewritten.
...
3.0.6 keeps Computer Use disabled on **Windows, macOS, and Linux**. It registers no computer-control tools, captures no screens, and performs no native mouse, keyboard, or window-control actions. ORCA Agent, automation, bots, and subtasks cannot start this feature.
...
- 3.0.6 focuses on the Modern navigation gutter and compact turn rail without crowding the transcript. Consecutive tool summaries merge while retaining actual stage text and its order, without generic status replacements or lost intermediate replies. This patch does not redesign the layout or change Classic.
...
| Local AI is missing | Managed runtime and downloads remain temporarily disabled in 3.0.6; old configuration cannot enable them. Files remain and custom external services still work |
...
See the [3.0.6 build checklist](docs/build/desktop-v3.0.6.md) and [acceptance record](docs/audits/desktop-v3.0.6-verification.md) for pending checks. The [3.0.5 verification](docs/audits/desktop-v3.0.5-verification.md), [3.0.4 validation report](docs/audits/2026-09-08-v3.0.4-release-validation.md), and [historical download measurements](docs/audits/2026-09-08-download-diagnosis.md) describe earlier results, not 3.0.6 acceptance. [desktop/README.md](desktop/README.md) and the [artifact runtime boundary](docs/ARTIFACT_RUNTIME.md) provide focused details.
